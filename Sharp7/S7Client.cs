...
namespace Sharp7
{
	public partial class S7Client
	{
		...
		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		public struct SystemStatusList
		{
		...
		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		public struct SystemStatusListUint
		{
			public SZL_HEADER Header;
		...
		#endregion [Constants and TypeDefs]
		...
		public byte[] FromTemplate(byte[] template)
		{
			byte[] copyOf = new byte[template.Length];
			Array.Copy(template, copyOf, template.Length);
		...
		{
			int Size;
			byte[] crt = FromTemplate(Telegrams.ISO_ConnectionRequestTelegram);

			crt[16] = LocalTSAP_HI;
			crt[17] = LocalTSAP_LO;
			crt[20] = RemoteTSAP_HI;
			crt[21] = RemoteTSAP_LO;
		...
		{
			int Length;
			byte[] tgram = FromTemplate(Telegrams.S7_PduNegotiationTelegram);

			// Set PDU Size Requested
			S7.SetWordAt(tgram, 23, (ushort)_PduSizeRequested);
		...
		private static int CpuError(ushort Error)
		{
			switch(Error)
			{
				case 0:
		...
		public int ReadArea(int Area, int DBNumber, int Start, int Amount, int WordLen, byte[] Buffer, ref int BytesRead)
		{
			...
			while((TotElements > 0) && (_LastError == 0))
			{
				...
				// Setup the telegram
				Array.Copy(Telegrams.S7_ReadWriteRequestHeader, 0, PDU, 0, Telegrams.HeaderSizeRead);
				...
				SendPacket(PDU, Telegrams.HeaderSizeRead);
				if(_LastError == 0)
				{
					Length = RecvIsoPacket();
					if(_LastError == 0)
		...
		public int WriteArea(int Area, int DBNumber, int Start, int Amount, int WordLen, byte[] Buffer, ref int BytesWritten)
		{
			...
			while((TotElements > 0) && (_LastError == 0))
			{
				...
				IsoSize = Telegrams.HeaderSizeWrite + DataSize;

				// Setup the telegram
				Array.Copy(Telegrams.S7_ReadWriteRequestHeader, 0, PDU, 0, Telegrams.HeaderSizeWrite);
		...
		public int ReadMultiVars(S7DataItem[] Items, int ItemsCount)
		{
			...
			// Fills Header
			Array.Copy(Telegrams.S7_VariableMultiReadHeader, 0, PDU, 0, Telegrams.S7_VariableMultiReadHeader.Length);
			...
			{
				Array.Copy(Telegrams.S7_VariableMultiReadItem, S7Item, S7Item.Length);
		...
		public int WriteMultiVars(S7DataItem[] Items, int ItemsCount)
		{
			...
			int DataLength;
			int ItemDataSize;
			byte[] S7ParItem = new byte[Telegrams.S7_VariableMultiWriteItem.Length];
			...
			// Fills Header
			Array.Copy(Telegrams.S7_VariableMultiWriteHeader, 0, PDU, 0, Telegrams.S7_VariableMultiWriteHeader.Length);
			ParLength = ItemsCount * Telegrams.S7_VariableMultiWriteItem.Length + 2;
			S7.SetWordAt(PDU, 13, (ushort)ParLength);
			PDU[18] = (byte)ItemsCount;

			// Fills Params
			Offset = Telegrams.S7_VariableMultiWriteHeader.Length;
			for(int c = 0; c < ItemsCount; c++)
			{
				Array.Copy(Telegrams.S7_VariableMultiWriteItem, 0, S7ParItem, 0, Telegrams.S7_VariableMultiWriteItem.Length);
				...
				Offset += Telegrams.S7_VariableMultiWriteItem.Length;
			}
		...
		#endregion [Data I/O main functions]
		...
		#endregion [Data I/O lean functions]
		...
		public int GetAgBlockInfo(int BlockType, int BlockNum, ref S7BlockInfo Info)
		{
			...
			Time_ms = 0;
			int Elapsed = Environment.TickCount;
			byte[] brh = FromTemplate(Telegrams.S7_GetBlockInfoRequestHeader);

			brh[30] = (byte)BlockType;

			// Block Number
			brh[31] = (byte)((BlockNum / 10000) + 0x30);
			BlockNum = BlockNum % 10000;
			brh[32] = (byte)((BlockNum / 1000) + 0x30);
			BlockNum = BlockNum % 1000;
			brh[33] = (byte)((BlockNum / 100) + 0x30);
			BlockNum = BlockNum % 100;
			brh[34] = (byte)((BlockNum / 10) + 0x30);
			BlockNum = BlockNum % 10;
			brh[35] = (byte)((BlockNum / 1) + 0x30);
		...
		public int GetPlcDateTime(ref DateTime DT)
		{
			...
			SendPacket(Telegrams.S7_GetDateTimeRequest);
			if(_LastError == 0)
			{
				Length = RecvIsoPacket();
		...
		{
			int Length;
			byte[] dtr = FromTemplate(Telegrams.S7_SetDateTimeRequest);
			_LastError = 0;
			Time_ms = 0;
			int Elapsed = Environment.TickCount;

			S7.SetDateTimeAt(dtr, 31, DT);
			SendPacket(dtr);
			if(_LastError == 0)
			{
				Length = RecvIsoPacket();
		...
		{
			SystemStatusList SZL = new SystemStatusList();
			int Size = 1024;
		...
		public int ReadSZL(int ID, int Index, ref SystemStatusList SZL, ref int Size)
		{
			int Length;
			int DataSZL;
			int Offset = 0;
			...
			byte[] ft = FromTemplate(Telegrams.S7_SslFirstTelegram);
			byte[] nt = FromTemplate(Telegrams.S7_SslNextTelegram);
			...
			do
			{
				if(First)
				{
					S7.SetWordAt(ft, 11, ++Seq_out);
					S7.SetWordAt(ft, 29, (ushort)ID);
					S7.SetWordAt(ft, 31, (ushort)Index);
					SendPacket(ft);
				}
				else
				{
					S7.SetWordAt(nt, 11, ++Seq_out);
					PDU[24] = (byte)Seq_in;
					SendPacket(nt);
				}
				if(_LastError != 0)
		...
		public int ReadSZLList(ref SystemStatusListUint List, ref Int32 ItemsCount)
		{
			return S7Consts.errCliFunctionNotImplemented;
		}
		...
		public int PlcHotStart()
		{
			...
			SendPacket(Telegrams.S7_HotStartRequest);
			if(_LastError == 0)
			{
				int Length = RecvIsoPacket();
				if(Length > 18) // 18 is the minimum expected
				{
					if(PDU[19] != Telegrams.pduStart)
						_LastError = S7Consts.errCliCannotStartPLC;
					else
					{
						if(PDU[20] == Telegrams.pduAlreadyStarted)
		...
		public int PlcColdStart()
		{
			...
			SendPacket(Telegrams.S7_ColdStartRequest);
			if(_LastError == 0)
			{
				int Length = RecvIsoPacket();
				if(Length > 18) // 18 is the minimum expected
		...
		public int PlcStop()
		{
			...
			SendPacket(Telegrams.S7_StopRequest);
			if(_LastError == 0)
			{
				int Length = RecvIsoPacket();
				if(Length > 18) // 18 is the minimum expected
				{
					if(PDU[19] != Telegrams.pduStop)
						_LastError = S7Consts.errCliCannotStopPLC;
					else
					{
						if(PDU[20] == Telegrams.pduAlreadyStopped)
		...
		public int PlcGetStatus(ref CpuStatus Status)
		{
			_LastError = 0;
			int Elapsed = Environment.TickCount;

			SendPacket(Telegrams.S7_GetStatus);
			if(_LastError == 0)
			{
				int Length = RecvIsoPacket();
				if(Length > 30) // the minimum expected
				{
					...
					if(Result == 0)
					{
						switch(PDU[44])
						{
							case (int)CpuStatus.Unknown:
							case (int)CpuStatus.Running:
							case (int)CpuStatus.Stopped:
								{
									Status = (CpuStatus)PDU[44];
									break;
								}
							default:
								{
		...
		public int SetSessionPassword(string Password)
		{
			...
			byte[] ssp = FromTemplate(Telegrams.S7_SetSessionPassword);
			int Length;
			_LastError = 0;
			int Elapsed = Environment.TickCount;
		...
		public int ClearSessionPassword()
		{
			...
			_LastError = 0;
			int Elapsed = Environment.TickCount;
			SendPacket(Telegrams.S7_ClearSessionPassword);
			if(_LastError == 0)
			{
				Length = RecvIsoPacket();
				if(Length > 30) // the minimum expected
		...
		{
			S7Client.SystemStatusList SZL = new S7Client.SystemStatusList();
			int Size = 256;
			SZL.Data = new byte[Size];
		...
		public int IsoExchangeBuffer(byte[] Buffer, ref Int32 Size)
		{
			...
			Time_ms = 0;
			int Elapsed = Environment.TickCount;
			Array.Copy(Telegrams.TPKT_IsoHeader, 0, PDU, 0, Telegrams.TPKT_IsoHeader.Length);
			S7.SetWordAt(PDU, 2, (ushort)(Size + Telegrams.TPKT_IsoHeader.Length));
			try
			{
				Array.Copy(Buffer, 0, PDU, Telegrams.TPKT_IsoHeader.Length, Size);
			}
			catch
			{
				return S7Consts.errIsoInvalidPDU;
			}
			SendPacket(PDU, Telegrams.TPKT_IsoHeader.Length + Size);
			if(_LastError == 0)
			{
				int Length = RecvIsoPacket();
				if(_LastError == 0)
				{
					Array.Copy(PDU, Telegrams.TPKT_IsoHeader.Length, Buffer, 0, Length - Telegrams.TPKT_IsoHeader.Length);
					Size = Length - Telegrams.TPKT_IsoHeader.Length;
				}
			}
			if(_LastError == 0)
				Time_ms = Environment.TickCount - Elapsed;
		...
		public int AsReadSZL(int ID, int Index, ref SystemStatusList Data, ref Int32 Size)
		{
			return S7Consts.errCliFunctionNotImplemented;
		}

		public int AsReadSZLList(ref SystemStatusListUint List, ref Int32 ItemsCount)
		{
			return S7Consts.errCliFunctionNotImplemented;
		}
		...
		#endregion [Async functions (not implemented)]
		...
		#endregion [Info Functions / Properties]
	}
}
